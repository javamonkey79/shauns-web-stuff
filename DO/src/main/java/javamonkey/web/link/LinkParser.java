...
public class LinkParser {
	...
	public String getLinkPageSource(String link, String cookieData,
			boolean useCache) throws Throwable {

		if (useCache) {
			File file = new File(System.getProperty("java.io.tmpdir"),
					URLEncoder.encode(link + ".html", "UTF-8"));
			if (!file.exists() || isFileOlderThanWeek(file)) {
				...
				LOG.info(String.format("Successfully wrote cache file: [%s]",
						file));
	...
	private boolean isFileOlderThanWeek(File file) {
		return Math.abs(Days.daysBetween(
				new LocalDateTime(file.lastModified()),
				new LocalDateTime(new Date())).getDays()) >= 7;
	}
